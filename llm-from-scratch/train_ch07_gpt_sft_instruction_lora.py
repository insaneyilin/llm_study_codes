...
def train_gpt():
    ...
    total_params_before = sum(p.numel() for p in gpt2_model.parameters()
                              if p.requires_grad)
    ...
    total_params_after_freeze = sum(p.numel() for p in gpt2_model.parameters()
                                    if p.requires_grad)
    print(
    ...
    total_params_lora = sum(p.numel() for p in gpt2_model.parameters()
                            if p.requires_grad)
    print(
    ...
    exp_name = "gpt2-instruction_sft-lora"
